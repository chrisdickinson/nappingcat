...
def add_user(request, username):
    if auth.has_permission(request.settings, request.user, ('auth', 'adduser')):
        auth.add_user(request.settings, username)
...
def add_key_to_user(request, username):
    if auth.has_permission(request.settings, request.user, ('auth', 'modifyuser')):
        key = request.stdin.read()
        settings = dict(request.settings.items('kittyconfig'))
        auth.add_key_to_user(request.settings, username, key) 
...
def add_permission(request, username, permission):
    ...
    if auth.has_permission(request.settings, request.user, ('auth', 'modifyuser')) or auth.has_permission(request.settings, request.user, permission_tuple):
        auth.add_permission(request.settings, username, permission_tuple)
...
def remove_permission(request, username, permission):
    ...
    if auth.has_permission(request.settings, request.user, ('auth', 'modifyuser')) or (user_owned_permission and auth.has_permission(request.settings, request.user, permission_tuple)):
        auth.remove_permission(request.settings, username, permission_tuple)
